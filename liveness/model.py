# MiniFASNet architecture — copied verbatim from the original repo.
# No changes needed: pure PyTorch nn.Module code, fully compatible with modern PyTorch.
import torch
...
# "1.8M" and "1.8M_" refer to approximate parameter counts (~1.8M each).
_KEEP = {
...
def MiniFASNetV1(embedding_size=128, conv6_kernel=(7, 7), drop_p=0.2, num_classes=3, img_channel=3):
    return MiniFASNet(_KEEP['1.8M'], embedding_size, conv6_kernel, drop_p, num_classes, img_channel)


def MiniFASNetV2(embedding_size=128, conv6_kernel=(7, 7), drop_p=0.2, num_classes=3, img_channel=3):
    return MiniFASNet(_KEEP['1.8M_'], embedding_size, conv6_kernel, drop_p, num_classes, img_channel)


def MiniFASNetV1SE(embedding_size=128, conv6_kernel=(7, 7), drop_p=0.75, num_classes=3, img_channel=3):
    return MiniFASNetSE(_KEEP['1.8M'], embedding_size, conv6_kernel, drop_p, num_classes, img_channel)


def MiniFASNetV2SE(embedding_size=128, conv6_kernel=(7, 7), drop_p=0.75, num_classes=4, img_channel=3):
    return MiniFASNetSE(_KEEP['1.8M_'], embedding_size, conv6_kernel, drop_p, num_classes, img_channel)
